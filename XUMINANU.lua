...
-- TTJY X
TTJY["1"] = Instance.new("ScreenGui", game:GetService("CoreGui"))
TTJY["1"].IgnoreGuiInset = true
TTJY["1"].ScreenInsets = Enum.ScreenInsets.DeviceSafeInsets
TTJY["1"].Name = "TTJY"
TTJY["1"].ZIndexBehavior = Enum.ZIndexBehavior.Sibling

TTJY["2"] = Instance.new("Frame", TTJY["1"])
TTJY["2"].BorderSizePixel = 0
TTJY["2"].BackgroundColor3 = Color3.fromRGB(42, 42, 58)
TTJY["2"].LayoutOrder = 2
TTJY["2"].BackgroundTransparency = 1
TTJY["2"].Size = UDim2.new(0.3, 0, 0.15, 0)
TTJY["2"].BorderColor3 = Color3.fromRGB(0, 0, 0)
TTJY["2"].Position = UDim2.new(0.7, 0, -0.03, 0)
TTJY["2"].Name = "Frame"

TTJY["3"] = Instance.new("TextLabel", TTJY["2"])
TTJY["3"].TextWrapped = true
TTJY["3"].BorderSizePixel = 0
TTJY["3"].TextScaled = true
TTJY["3"].BackgroundColor3 = Color3.fromRGB(255, 255, 255)
TTJY["3"].BackgroundTransparency = 1
TTJY["3"].TextXAlignment = Enum.TextXAlignment.Left
TTJY["3"].Font = Enum.Font.SourceSansSemibold
TTJY["3"].TextSize = 14
TTJY["3"].TextColor3 = Color3.fromRGB(255, 0, 0)
TTJY["3"].Size = UDim2.new(1, 0, 1, 0)
TTJY["3"].Position = UDim2.new(0, 0, 0, 0)
TTJY["3"].Text = "TTJY HUB"

TTJY["4"] = Instance.new("UIStroke", TTJY["3"])
...
-- DrRay.TopBar.ProfileMenu.LeftButton
DRR["Left"] = Instance.new("ImageButton", DRR["c"])
DRR["Left"]["BorderSizePixel"] = 0
DRR["Left"]["AutoButtonColor"] = false
DRR["Left"]["BackgroundColor3"] = Color3.fromRGB(42, 42, 58)
DRR["Left"]["Size"] = UDim2.new(0.05, 0, 0.682426393032074, 0) -- Adjust the size as needed
DRR["Left"]["Name"] = [[Left]]
DRR["Left"]["BorderColor3"] = Color3.fromRGB(0, 0, 0)
DRR["Left"]["Position"] = UDim2.new(0.015024710446596146, 0, 0.18421050906181335, 0)

-- DrRay.TopBar.ProfileMenu.RightButton
DRR["Right"] = Instance.new("ImageButton", DRR["c"])
DRR["Right"]["BorderSizePixel"] = 0
DRR["Right"]["AutoButtonColor"] = false
DRR["Right"]["BackgroundColor3"] = Color3.fromRGB(42, 42, 58)
DRR["Right"]["Size"] = UDim2.new(0.05, 0, 0.682426393032074, 0) -- Adjust the size as needed
DRR["Right"]["Name"] = [[Right]]
DRR["Right"]["BorderColor3"] = Color3.fromRGB(0, 0, 0)
DRR["Right"]["Position"] = UDim2.new(0.95, 0, 0.18421050906181335, 0) -- Adjust the position to be on the right
...
Closure = function()
	...
	local LeftDragInput = false;
	local RightDragInput = false;
	
	local LeftSpeed = 0;
	local RightSpeed = 0;
	
	local function GetSpeed(Arg)
		if Arg == "L" then
			local succes : boolean, response : any = pcall(function()
				local Speed = 0;
				if tick() - LeftSpeed <= 5 then
					local L = tick() - LeftSpeed
					Speed = 5 + L;
				end;
				return math.clamp(Speed, 5, 15);	
			end);
			if (succes) then
				return response;
			end;
		elseif Arg == "R" then
			local succes : boolean, response : any = pcall(function()
				local Speed = 0;
				if tick() - RightSpeed <= 5 then
					local R = tick() - RightSpeed
					Speed = 5 + R;
				end;
				return math.clamp(Speed, 5, 15);
			end);
...
local function TabsScrollLeft()
	local Scroll = DRR["4"]; 
	local Speed = GetSpeed("L");
	Scroll.CanvasPosition = Vector2.new(Scroll.CanvasPosition.X - Speed, 0)
end
local function TabsScrollRight()
	local Scroll = DRR["4"]; 
	local Speed = GetSpeed("R");
	Scroll.CanvasPosition = Vector2.new(Scroll.CanvasPosition.X + Speed, 0)
end

DRR["Left"].InputBegan:Connect(function(input)
...
DRR["Right"].InputBegan:Connect(function(input)
	if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
		RightDragInput = true
		RightSpeed = tick();
		input.Changed:Connect(function()
			if input.UserInputState == Enum.UserInputState.End then
				RightDragInput = false
				RightSpeed = 0;
			end
		end)	
	end
...
local theard = task.spawn(function()
	while task.wait() do 
		if LeftDragInput == true then
			TabsScrollLeft();
		end;
		if RightDragInput == true then
			TabsScrollRight();
		end;
	end;	
end);
